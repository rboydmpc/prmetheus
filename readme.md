## Grafana - Prometheus for Morpheus HA Monitoring  

The following components will be required and setup on all Morpheus App Nodes.

- Docker installed on the Morpheus App Nodes.
- A Docker shared network created
...
[RHEL Install Doc](https://docs.docker.com/engine/install/rhel/) \
[Ubuntu Install Doc](https://docs.docker.com/engine/install/ubuntu/)
...
This command will look at the root file system at /etc/prometheus/mysql_exporter.cnf for authentication to MySQL. Make sure you have this file in place with the appropriate rights. 

A sample of this file can be found in this repo.
...
```
docker run -d -v /etc/prometheus:/etc/prometheus -p 127.0.0.1:9104:9104 --network prometheus --name mysql_exporter prom/mysqld-exporter --config.my-cnf=/etc/prometheus/mysql_exporter.cnf --collect.perf_schema.replication_group_members --collect.perf_schema.replication_group_member_stats
```

Confirm container is running without issue
...
**Modify the below command to add your Morpheus App Node IP in place of 192.x.x.x. Since we are not running the container on the host network dont use 127.0.0.1 but the actual ip**

```
docker run --rm -d --network prometheus --name=elasticsearch_exporter -p 127.0.0.1:9114:9114 bitnami/elasticsearch-exporter:latest --es.uri=http://192.x.x.x:9200
```
...
```
docker run -d -p 127.0.0.1:9100:9100 --network prometheus --pid="host" -v "/:/host:ro,rslave"  --name=node_exporter --path.rootfs=/host prom/node-exporter --path.rootfs=/host
```
...
## Enable RabbitMQ Exporter

RabbitMQ has a built in plugin for PRometheus. The following steps will show how to enable it with Morpheus Embedded RabbitMQ.
\
First source rabbitmq

```
source /opt/morpheus/embedded/rabbitmq/.profile
```
\
Create a cluster name (This will ensure that the grafana dashboard sees all nodes as part of the same cluster)
```
rabbitmqctl -q set_cluster_name morphclus
```
\
Enable the plugin
```
...
First create the config file on the root filesystem at /etc/prometheus/prometheus.yml

Sample of Prometheus config file. **Make sure to change the db1, db2, db3, rabbit_node1, rabbit_node2 and rabbit_node3 192.x.x.x to your ip addresses.**
...
<details>
  ...
  scrape_configs:
    ...
    - job_name: 'db1'
      metrics_path: /probe
      #params:
      #  module: [server]  # auth_module parameter for the probe
      static_configs:
        - targets:
            - '192.x.x.x:3306'  # MySQL exporter target
      relabel_configs:
        - source_labels: [__address__]
          target_label: __param_target
        - target_label: __address__
          replacement: mysql_exporter:9104  # Blackbox exporter address
    ...
    - job_name: rabbit_node1
      static_configs:
        - targets: ['192.x.x.x:15692']
  
    - job_name: rabbit_node2
      static_configs:
        - targets: ['192.x.x.x:15692']
  
    - job_name: rabbit_node3
      static_configs:
        - targets: ['192.x.x.x:15692']
  ```
</details>
...
```
docker run --name prometheus --network prometheus -d -p 127.0.0.1:9090:9090 -v /etc/prometheus:/etc/prometheus prom/prometheus --config.file=/etc/prometheus/prometheus.yml --storage.tsdb.path=/prometheus --storage.tsdb.retention.time=30d
```
...
## Install Grafana
This will create a clustered Grafana install running in Docker across the 3 Morpheus App nodes.

- Create a user that has the appropriate rights in MySQL for Grafana

- Create a grafana database and give this user permissions to it.

- Restore the database located in this repo to your MySQL. This will load p the initial dataset with pre defined dashboards. user:admin, password:P@ssw0rd!

- Create the config file on the root filesystem at /etc/grafana/grafana.ini

Sample of Grafana config file. **Make sure to change the database section to your MySQL information the 192.x.x.x addresses should be changed to your IP address of your local Morpheus app node running MySQL Router.**

<details>
  <summary>grafana.ini</summary>
...
```
docker run --name=grafana --network prometheus -d -p 3000:3000 -v /etc/grafana:/etc/grafana grafana/grafana
```
...
## Log into Grafana and confirm
you should now be able to log into any of the grafana instances running on the app nodes. http://192.x.x.x:3000
